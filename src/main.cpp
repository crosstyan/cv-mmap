...
int main(int argc, char **argv) {
	...
	backend->SetOnFrame([&frame_state, &sync_msg, &sock, &shm_state, &build_v2_metadata](std::span<uint8_t> frame_buffer, const frame_metadata_t &metadata) {
		...
		std::copy_n(frame_buffer.begin(), metadata_v2->header.payload_size_bytes, fs.image_buffer().begin());
