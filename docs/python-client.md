...
- the protobuf control/status contract in `core/proto/cvmmap/control.proto`
...
The Python package is not expected to link the C++ library. Its correctness comes from keeping its own protocol test suite in sync with the same ABI and URI contract for live transports.

For cross-repo parser checks, the Python tests can read the canonical binary
fixtures generated by `core/tests/protocol_fixture_writer.cpp` from
`core/fixtures/protocol/`.
