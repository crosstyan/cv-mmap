...
void UdpRtpBackend::SetOnBodyTracking(on_body_tracking_fn_t on_body_tracking) {
	(void)on_body_tracking;
}
...
cvmmap::expected<recording_status_t, error_t> UdpRtpBackend::StartRecording(std::string_view output_path) {
	(void)output_path;
	return cvmmap::unexpected(-ENOTSUP);
}

cvmmap::expected<recording_status_t, error_t> UdpRtpBackend::StopRecording() {
	return cvmmap::unexpected(-ENOTSUP);
}

cvmmap::expected<recording_status_t, error_t> UdpRtpBackend::GetRecordingStatus() {
	return cvmmap::unexpected(-ENOTSUP);
}

std::string UdpRtpBackend::GetLastRecordingError() {
	return {};
}
