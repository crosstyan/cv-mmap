...
struct ZedBackendImpl {
	...
	std::optional<cvmmap::body_tracking_frame_t> capture_body_tracking_frame_locked(
		...
		if (retrieve_result != sl::ERROR_CODE::SUCCESS) {
			spdlog::warn("ZED retrieveBodies error: code={}", static_cast<int>(retrieve_result));
			return std::nullopt;
		}
	...
	std::optional<size_t> pack_frame_locked(
		...
		if (has_depth_payload) {
			...
			if (!depth_plane_available) {
				if (depth_result != sl::ERROR_CODE::SUCCESS) {
					spdlog::warn("ZED retrieveMeasure(DEPTH) error: code={}; using stable fallback depth bytes", static_cast<int>(depth_result));
		...
		if (has_depth_payload) {
			...
			if (depth_plane_available && copy_compact_plane(depth_frame, depth_row_bytes, depth_payload)) {
				...
				if (depth_plane_available) {
					spdlog::warn("ZED depth plane compaction error; using stable fallback depth bytes");
				}
		...
		if (packed_confidence_size > 0) {
			...
			if (!copy_compact_plane(confidence_frame, depth_row_bytes, confidence_payload)) {
				spdlog::warn("ZED confidence plane compaction error; publishing left/depth only");
	...
	cvmmap::expected<CapturedFrame, sl::ERROR_CODE> capture_frame_locked(
		...
		if (retrieve_result != sl::ERROR_CODE::SUCCESS) {
			spdlog::debug("ZED retrieveImage error: code={}", static_cast<int>(retrieve_result));
	...
	void worker_loop(std::stop_token stop_token) {
		...
		while (!stop_token.stop_requested()) {
			...
			if (!reconnect()) {
				on_error(-EIO, "ZED reconnect error");
				continue;
			}
			consecutive_failures = 0;
		}
	...
	cvmmap::expected<recording_status_t, error_t> StartRecording(const svo_recording_request_t &request) {
		...
		if (error_message) {
			...
			spdlog::error("ZED recording start error for '{}': {}", request.output_path, *error_message);
